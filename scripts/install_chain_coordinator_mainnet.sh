...
jq '.app_state.mint.minter.inflation = "0.030000000000000000"' $GENESIS > temp.json && mv temp.json $GENESIS
jq '.app_state.mint.params.inflation_rate_change = "0.030000000000000000"' $GENESIS > temp.json && mv temp.json $GENESIS
jq '.app_state.mint.params.inflation_max = "0.050000000000000000"' $GENESIS > temp.json && mv temp.json $GENESIS
...
(echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | mirumd genesis gentx validator "1000000000000$STAKE" --chain-id="$CHAIN_ID" --amount="1000000000000$STAKE" --home "$HOMEP"
## should be:
